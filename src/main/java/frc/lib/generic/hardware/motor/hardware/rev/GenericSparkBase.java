...
public abstract class GenericSparkBase extends Motor {
    ...
    public void setOutput(MotorProperties.ControlMode mode, double output, double feedforward) {
        if (mode == MotorProperties.ControlMode.VOLTAGE && isDuplicateOutput(mode, output, feedforward))
            return;
        ...
        switch (mode) {
            ...
            case VOLTAGE -> sparkController.setSetpoint(output, SparkBase.ControlType.kVoltage, ClosedLoopSlot.kSlot0, 0);
            case CURRENT -> sparkController.setSetpoint(output, SparkBase.ControlType.kCurrent, ClosedLoopSlot.kSlot0, 0);
        }
    }
